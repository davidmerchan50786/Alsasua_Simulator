...
public static class SetupEscenaAlsasua
{
    ...
    // Y=0  → nivel de calle (altura geográfica 530 m)
    ...
    static void AsegurarCamara()
    {
        ...
        if (cam == null)
        {
            ...
            // NO añadimos AudioListener a la Main Camera (cámara dron a Y=1500).
            // CamaraFPS (hija del Jugador) es la cámara activa de gameplay y ya tiene
            // su propio AudioListener. Tener dos AudioListeners activos genera warning
            // de Unity y puede causar comportamientos de audio inesperados.
        ...
        else
        {
            ...
            // Eliminar CesiumOriginShift si lo tiene (bug heredado: estaba en la
            // cámara a Y=1500 haciendo que Cesium cargara tiles aéreos, no de suelo)
            var cos = go.GetComponent<CesiumOriginShift>();
            if (cos != null)
            {
                Object.DestroyImmediate(cos);
                Debug.Log("[Setup] ✓ CesiumOriginShift eliminado de Main Camera (bug corregido: ahora va en Jugador)");
            }
        }

        cam.nearClipPlane = 0.3f;
        cam.farClipPlane  = 1_000_000f;  // 1M m — Cesium requiere far muy grande para ver el terreno real
        cam.fieldOfView   = 60f;
        cam.depth         = -10f;  // renderiza por detrás de CamaraFPS (priority 0)
    ...
    static void AsegurarJugador()
    {
        ...
        cam.farClipPlane  = 1_000_000f;  // 1M m — obligatorio para ver tiles de Cesium en el horizonte
        cam.fieldOfView   = 70f;
