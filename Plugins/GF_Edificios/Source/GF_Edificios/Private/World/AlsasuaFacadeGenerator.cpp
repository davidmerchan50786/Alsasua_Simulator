...
bool UAlsasuaFacadeGenerator::MedidasDe(int32 IdEdificio, float& AlturaPorNivelM,
    float& AnchoVentanaM, float& AltoVentanaM) const
{
    const FBuildingFacadeEntry* Fach = De(IdEdificio);
    if (!Fach)
    {
        return false;
    }
    AlturaPorNivelM = Fach->AlturaPorNivel;
    AnchoVentanaM = 0.f;
    AltoVentanaM = 0.f;
    if (Fach->Ventanas.Num() > 0)
    {
        float SumaAncho = 0.f, SumaAlto = 0.f;
        for (const FWindowData& V : Fach->Ventanas)
        {
            SumaAncho += V.Ancho;
            SumaAlto += V.Alto;
        }
        AnchoVentanaM = SumaAncho / Fach->Ventanas.Num();
        AltoVentanaM = SumaAlto / Fach->Ventanas.Num();
    }
    return true;
}

//~ IAlsasuaPilarArranque (fase 21 del antiguo DirectorArranque)
int32 UAlsasuaFacadeGenerator::EjecutarArranque()
{
	return ColocarLandmarksReales() + ColocarParadasTransporte();
}

FString UAlsasuaFacadeGenerator::EtiquetaArranque() const
{
	return TEXT("landmarks y paradas de transporte");
}

int32 UAlsasuaFacadeGenerator::OrdenArranque() const
{
	return 210;
}
