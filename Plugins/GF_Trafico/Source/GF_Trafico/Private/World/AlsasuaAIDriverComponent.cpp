...
{
	UAlsasuaStreetGraph* G = ResolverGrafo();
	AActor* Owner = GetOwner();
	if (!G || G->Nodes.Num() < 2 || !Owner) return;

	SetRouteFromGraph(G, G->GetNearestNode(Owner->GetActorLocation()), G->GetNearestNode(Dest));
	if (RoutePoints.Num() > 0) RoutePoints.Last() = Dest;
}

void UAlsasuaAIDriverComponent::SetRouteFromGraph(UAlsasuaStreetGraph* Graph, int32 StartNode, int32 EndNode)
{
	RoutePoints.Empty();
	CurrentRouteIndex = 0;
	...
	const TArray<int32> Camino = Graph->FindPath(StartNode, EndNode);
	if (Camino.Num() < 2) return;

	RoutePoints = Graph->GetRoutePoints(Camino);
...
UAlsasuaStreetGraph* UAlsasuaAIDriverComponent::ResolverGrafo()
{
	...
	if (const UGameInstance* GI = World->GetGameInstance())
		if (UAlsasuaDynamicTrafficSystem* Trafico = GI->GetSubsystem<UAlsasuaDynamicTrafficSystem>())
			Grafo = Trafico->ObtenerGrafo(); // lo carga si aún no existe
	return Grafo;
}
...
void UAlsasuaAIDriverComponent::NuevaRutaAleatoria()
{
	...
	const int32 Inicio = G->GetNearestNode(Owner->GetActorLocation());
	for (int32 Intento = 0; Intento < 8; ++Intento)
	{
		int32 Fin = FMath::RandRange(0, G->Nodes.Num() - 1);
		if (Fin == Inicio) { Fin = (Inicio + 1) % G->Nodes.Num(); }
		SetRouteFromGraph(G, Inicio, Fin);
		if (RoutePoints.Num() >= 2) return;
	}
}
