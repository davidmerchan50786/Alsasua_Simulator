...
bool UGuardDetectionComponent::IsInVisionCone(const AActor* Target) const
{
    ...
    // Radio de visión efectivo: se reduce si el jugador lleva disfraz.
    float EffectiveRange = VisionRange;
    if (const UDisguiseComponent* Disguise = Target->FindComponentByClass<UDisguiseComponent>())
    {
        EffectiveRange *= Disguise->GetEffectiveDetectionMultiplier();
    }
...
void UGuardDetectionComponent::ReportNoise(FVector NoiseLocation, float Loudness)
{
    ...
    if (IsValid(CachedPlayerTarget))
    {
        if (const UDisguiseComponent* Disguise = CachedPlayerTarget->FindComponentByClass<UDisguiseComponent>())
        {
            EffectiveLoudness *= Disguise->GetNoiseReduction();
        }
    }
